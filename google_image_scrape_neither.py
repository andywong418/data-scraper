...
with open("neither_images_link.json", "w") as outfile:
...
for index,img in enumerate(image_array):
    ...
    try:
        ...
        print img[1]
        if len(img[1]) == 0:
            f = open(os.path.join(DIR, "neither_image" + "_" + str(counter) + '.jpg'), 'wb')
        else:
            if img[1] == 'ashx':
                f = open(os.path.join(DIR, "neither_image" + "_" + str(counter) + '.jpg'), 'wb')
            else:
                f = open(os.path.join(DIR, "neither_image" + "_" + str(counter) + '.' + img[1]), 'wb')
        f.write(raw_img)
        f.close()
    except Exception as e:
